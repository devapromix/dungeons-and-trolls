...
function shop.display_interior(shop_type, player)
	...
	if interior.id ~= "" then
		...
		output.add(shop.get_trainable_skills_string(interior))
	end
...
function shop.get_trainable_skills_string(interior)
	if not interior.trainable_skills or #interior.trainable_skills == 0 then
		return ""
	end
	
	local skill_list = {}
	for _, skill in ipairs(interior.trainable_skills) do
		table.insert(skill_list, skill.name .. " (" .. skill.price .. " gold)")
	end
